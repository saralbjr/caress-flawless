...
export default function QuizStepThree() {
  ...
  const handleCurrentProductsChange = (value: string[]) => {
    setValue("routinePreferences.currentProducts", value, {
      shouldValidate: true,
    });
  ...
  const handlePreferredIngredientsChange = (value: string[]) => {
    setValue("routinePreferences.preferredIngredients", value, {
      shouldValidate: true,
    });
  ...
  const handleAvoidIngredientsChange = (value: string[]) => {
    setValue("routinePreferences.avoidIngredients", value, {
      shouldValidate: true,
    });
  ...
  const handleBudgetChange = (e: any) => {
    setValue("routinePreferences.budget", e.target.value, {
      shouldValidate: true,
    });
  ...
  const handleComplexityChange = (e: any) => {
    setValue("routinePreferences.routineComplexity", e.target.value, {
      shouldValidate: true,
    });
  ...
  const updateQuizResponses = (field: string, value: any) => {
    ...
    const questionIndex = existingResponses.findIndex(
  ...
  const handleInputConfirm = () => {
    if (inputValue && !currentProducts.includes(inputValue)) {
      ...
      setValue("routinePreferences.currentProducts", updatedProducts, {
        shouldValidate: true,
      });
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-8">
        ...
        <Paragraph>
          Tell us about your current routine and preferences to help us
          recommend products that will work best for you.
        </Paragraph>
      </div>
      ...
      <div className="space-y-4">
        ...
        <Paragraph>
          Select ingredients you prefer to avoid in your skincare products.
          (Optional)
        </Paragraph>
      ...
      <div className="space-y-4">
        ...
        <Radio.Group onChange={(e) => handleBudgetChange(e)} value={budget}>
          <div className="space-y-2">
            <Radio value="low">Budget-Friendly (Under Rs.20 per product)</Radio>
            <Radio value="medium">Mid-Range (Rs.20-Rs.50 per product)</Radio>
            <Radio value="high">Premium (Rs.50+ per product)</Radio>
          </div>
        </Radio.Group>
      </div>
